...
-----
 ...
 SELECT customers.email FROM customers LEFT JOIN orders ON(customers.id = orders.customer_id) GROUP BY customers.id HAVING SUM(orders.num_cupcakes) > 100 ORDER BY customers.email DESC;
